 ...
 * @author: Song
 * @create: Created in 2019-01-14 14:36
 * @Modified by:
 **/
public class Code_438_FindAllAnagramsInString {
    public static List<Integer> findAnagrams(String s, String p) {
        List<Integer> list = new ArrayList<>();
        HashMap<Character, Integer> pmap = new HashMap<>();
        HashMap<Character,Integer> help;
        ...
        for (int i = 0; i < s.length() - p.length() + 1; i++){
            ...
            for (int j = 0; j < p.length(); j++){
                if (pmap.containsKey(s.charAt(i + j))){
                    if (help.containsKey(s.charAt(i +j))){
                        if (help.get(s.charAt(i + j)) >= pmap.get(s.charAt(i + j))){
                            break;
                        }
                        help.put(s.charAt(i + j),help.get(s.charAt(i + j)) + 1);
                    }else {
                        help.put(s.charAt(i + j), 1);
                    }
                }else {
                    i = i + j;
                    break;
